...
у неё прозрачный, а когда играть нечего — она пропадает из кадра.

Вид настраивается: обложка и её размер, подложка (тёмная, светлая или
совсем без неё — тогда остаётся только текст с обводкой), полоса
времени, время числами, цвет полосы, размер текста и наибольшая ширина.
Изменения видно сразу, перезагружать источник в OBS не нужно.
...
- **Свернуть в трей вместо закрытия**, автозапуск с системой и старт
  свёрнутым
- **Управление этим компьютером с телефона и колонки** — в дополнение
  к управлению другими устройствами отсюда
